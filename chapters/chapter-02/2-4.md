...
- [**`data/users.js`**](data-usersjs): Stores the in-memory `users` array and `nextId`.
- **`controllers/userController.js`**: Contains CRUD logic.
- **`middleware/`**: Holds validation and existence check middleware.
- **`routes/userRoutes.js`**: Defines RESTful routes.
- **`app.js`**: Sets up middleware and mounts routes.
